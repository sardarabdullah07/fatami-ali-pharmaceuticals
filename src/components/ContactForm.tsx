...
/**
 * Submissions are delivered by Web3Forms to the company inbox — the site has
 * no server of its own, so the form posts straight to their relay. If that
 * relay cannot be reached, the form does not pretend: it falls back to
 * composing the message for the visitor's own email app, addressed to both
 * company inboxes, and says so. Nobody is told "sent" when nothing was.
 *
 * `Values` is the payload, `validate` is pure, and `submit` is the single
 * place the delivery mechanism lives.
 */
...
type Outcome = { kind: 'sent' } | { kind: 'fallback'; mailto: string }

const ENDPOINT = 'https://api.web3forms.com/submit'

/** The mail-app path: the same message, addressed to both inboxes. */
...
/**
 * The single integration point. Posts to the relay; anything short of a
 * confirmed success — a network failure, a non-2xx, a `success: false` —
 * becomes the fallback rather than a false "sent".
 */
async function submit(values: Values): Promise<Outcome> {
  try {
    const res = await fetch(ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        // Web3Forms uses `from_name` as the sender line and `email` as reply-to.
        from_name: `${values.name} — ${values.organization}`,
        email: values.email,
        name: values.name,
    ...
    // Offline, blocked, or the relay is down — fall through.
  }
  return { kind: 'fallback', mailto: composeMailto(values) }
}
...
export function ContactForm() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    // A filled honeypot is a bot. Show it a success and send nothing.
        ...
        {done ? (
            ...
            {done.kind === 'sent' ? (
              <>
                <CheckCircle2
                  ...
                  aria-hidden="true"
                />
                <h3 className="mt-6 text-display-sm text-fg">Your inquiry has been sent</h3>
                <p className="mx-auto mt-4 max-w-md text-pretty text-body-sm leading-relaxed text-fg-muted">
                  Thank you. It has been delivered to our team in Kabul, and we will reply to{' '}
                  <span className="font-medium text-fg">{values.email}</span>.
                </p>
              ...
              <>
                <Mail className="mx-auto h-11 w-11 text-accent" strokeWidth={1.25} aria-hidden="true" />
                <h3 className="mt-6 text-display-sm text-fg">We could not reach our mail service</h3>
                <p className="mx-auto mt-4 max-w-md text-pretty text-body-sm leading-relaxed text-fg-muted">
                  Your message has not been lost. The button below opens it in your own email
                  app, already addressed to both of our inboxes and filled in — review it and
                  press send.
                </p>
                <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
                  <a
          ...
          >
            ...
            {/* Honeypot. Off-screen and out of the tab order, so a person never
                meets it; a bot filling every field it finds trips it. */}
            ...
            <div className="flex flex-col gap-4 border-t border-line pt-6 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-[0.8125rem] leading-relaxed text-fg-subtle">
                Delivered to our team in Kabul. We reply to the address you give.
              </p>
              <button
                type="submit"
                disabled={sending}
                aria-busy={sending}
                className="group inline-flex min-h-[52px] shrink-0 cursor-pointer items-center justify-center gap-2 rounded-full bg-action px-7 py-3 text-[0.9375rem] font-semibold text-on-action transition-all duration-300 hover:bg-action-hover hover:shadow-action active:translate-y-px disabled:cursor-wait disabled:opacity-80 disabled:hover:shadow-none"
              >
                {sending ? (
                  <>
                    Sending
